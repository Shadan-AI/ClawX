...
/**
 * Re-apply bundled models.providers.* baseUrl/api when the Gateway later
 * overwrites openclaw.json with a dev-LAN endpoint. Used by the config watcher.
 */
export async function reconcileOpenClawModelsProvidersFromBundledTemplate(): Promise<boolean> {
  // No-op stub: this project uses a fixed shadan provider URL, not a bundled template.
  // The watcher calls this as a best-effort guard; returning false is safe.
  return false;
}
