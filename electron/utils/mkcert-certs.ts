/**
 * Windows: generate ~/.openclaw/certs/localhost.pem + localhost-key.pem via
 * the mkcert.exe bundled inside the @shadanai/openclaw npm package.
 *
 * On first run, always force-regenerates certs and installs the root CA so
 * the system trusts them. Completion is recorded in ~/.openclaw/.env as
...
/** Private IPv4 (RFC1918) addresses for LAN HTTPS. */
 ...
 * - Development: node_modules/@shadanai/openclaw/mkcert.exe
 */
...
export async function ensureOpenClawMkcertCertsWindows(options: MkcertEnsureOptions = {}): Promise<MkcertEnsureResult> {
  ...
  if (!mkcert) {
    const msg = 'mkcert.exe not found in openclaw package (node_modules/@shadanai/openclaw/mkcert.exe)';
