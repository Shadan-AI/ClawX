...
pnpm package:mac          # macOS向けにパッケージ化（macOS 11/12 は zip のみ）
pnpm package:mac:dmg      # macOS で dmg + zip を強制生成
pnpm package:win          # Windows向けにパッケージ化
pnpm package:linux        # Linux向けにパッケージ化
```

macOS 11 / 12 では、`electron-builder` に同梱される `dmgbuild` ランタイムが `mkfifoat` に依存しており、このシンボルは macOS 13+ でしか利用できないため、`pnpm package:mac` は既定で DMG ターゲットをスキップします。互換性のあるカスタム `dmgbuild` を用意している場合は、`pnpm package:mac:dmg`（または `CLAWX_FORCE_DMG=1`）で DMG 生成を強制できます。
