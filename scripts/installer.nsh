; OpenMe Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting OpenMe runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers OpenMe.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if OpenMe.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old OpenMe was installed
  ...
  ; "Can't modify OpenMe's files" retry loop -> "OpenMe 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "OpenMe 无法关闭" dialog.
...
; "OpenMe 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with OpenMe's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove OpenMe application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\openme (local app data)$\r$\n  • AppData\Roaming\openme (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering OpenMe processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\openme"
    RMDir /r "$APPDATA\openme"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\openme
    IfFileExists "$LOCALAPPDATA\openme\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\openme"
      IfFileExists "$LOCALAPPDATA\openme\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\openme"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\openme
    IfFileExists "$APPDATA\openme\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\openme"
      IfFileExists "$APPDATA\openme\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\openme"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\openme\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\openme"
    IfFileExists "$APPDATA\openme\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\openme"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\openme"
    RMDir /r "$R3\AppData\Roaming\openme"
