...
# Post-installation script for OpenMe on Linux
...
# Create symbolic link for OpenMe app binary
if [ -x /opt/OpenMe/openme ]; then
    ln -sf /opt/OpenMe/openme /usr/local/bin/openme 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/OpenMe/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/OpenMe/chrome-sandbox' || true
else
    chmod 0755 '/opt/OpenMe/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/OpenMe/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/openme'
...
echo "OpenMe has been installed successfully."
