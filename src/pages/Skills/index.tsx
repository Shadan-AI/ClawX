...
export function Skills() {
  ...
  const installedCount = safeSkills.length;
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [officialQuery, skillsTab, searchSkills]);

  useEffect(() => {
    if (skillsTab !== 'market') return;
    const q = marketQuery.trim();
    if (!q) {
      void searchMarketSkills('', false);
      return;
    }
    const timer = setTimeout(() => {
      void searchMarketSkills(q, false);
    }, 300);
    return () => clearTimeout(timer);
  }, [marketQuery, skillsTab, searchMarketSkills]);
  ...
  const handleInstallMarket = useCallback(async (slug: string) => {
    try {
      await installMarketSkill(slug);
      const s = useSkillsStore.getState().skills.find((x) => x.slug === slug || x.id === slug);
      if (s) {
  ...
  }, [installMarketSkill, enableSkill, t]);
            ...
            <div className="relative group flex items-center bg-black/5 dark:bg-white/5 rounded-full px-3 py-1.5 focus-within:bg-black/10 transition-colors border border-transparent focus-within:border-black/10 dark:focus-within:border-white/10 mr-2">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (skillsTab === 'installed') setSearchQuery(v);
                  else if (skillsTab === 'official') setOfficialQuery(v);
                  else setMarketQuery(v);
                }}
                className="ml-2 bg-transparent outline-none w-28 md:w-48 font-normal placeholder:text-foreground/50 text-[13px] text-foreground"
              />
              {(skillsTab === 'installed' ? searchQuery : skillsTab === 'official' ? officialQuery : marketQuery) ? (
                <button
                  type="button"
                  onClick={() => {
                    if (skillsTab === 'installed') setSearchQuery('');
                    else if (skillsTab === 'official') setOfficialQuery('');
                    else setMarketQuery('');
                  }}
            ...
            <div className="flex items-center gap-6">
              <button
                ...
                className={cn(
                  ...
                  skillsTab === 'installed' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground',
                )}
              >
                {t('tabs.installed')} ({installedCount})
              </button>
              <button
                type="button"
                onClick={() => setSkillsTab('official')}
                className={cn(
                  'font-medium transition-colors flex items-center gap-1.5',
                  skillsTab === 'official' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground',
                )}
              >
                {t('tabs.official')}
              </button>
              <button
                type="button"
                onClick={() => setSkillsTab('market')}
                className={cn(
                  'font-medium transition-colors flex items-center gap-1.5',
                  skillsTab === 'market' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground',
                )}
              >
                {t('tabs.market')}
              </button>
        ...
        <div className="flex-1 overflow-y-auto pr-2 pb-10 min-h-0 -mr-2">
          ...
          {skillsTab === 'official' && (
            <div className="flex flex-col gap-1">
              ...
              {!searching &&
                ...
                !searchError &&
                officialQuery.trim().length >= 2 && (
                  <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
                    <Package className="h-10 w-10 mb-4 opacity-50" />
                    <p>{t('marketplace.noResults')}</p>
                  </div>
                )}
              {!searching && officialQuery.trim().length < 2 && (
                <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                  <Package className="h-10 w-10 mb-4 opacity-50" />
                  <p className="text-center max-w-md">
                    {officialQuery.trim().length > 0 ? t('officialMinChars') : t('marketplace.emptyPrompt')}
                  </p>
                </div>
              )}
            </div>
          ...
          {skillsTab === 'market' && (
            <div className="flex flex-col gap-1">
                      ...
                      >
                        <div className="flex items-start gap-4 flex-1 overflow-hidden pr-4 min-w-0">
                          ...
                          <div className="flex flex-col overflow-hidden min-w-0">
                            ...
                            {row.summary ? (
                              <p className="text-[13.5px] text-muted-foreground line-clamp-2 pr-2 leading-relaxed">
                                {row.summary}
                              </p>
                            ) : null}
                            <span className="text-[11px] font-mono text-muted-foreground mt-1">{row.slug}</span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          {isInstalled ? (
                            <span className="text-xs text-muted-foreground px-2">{t('marketplace.installedBadge')}</span>
                          ) : (
                            <Button
                              variant="default"
                              size="sm"
                              onClick={() => void handleInstallMarket(row.slug)}
                              disabled={busy || !isGatewayRunning}
                              className="h-8 px-4 rounded-full shadow-none font-medium text-xs"
                            >
                              {busy ? <LoadingSpinner size="sm" /> : t('marketplace.install', 'Install')}
                            </Button>
                          )}
                        </div>
                      </div>
              ...
              {!marketLoading && marketResults.length === 0 && !marketError && (
                <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
                  <Package className="h-10 w-10 mb-4 opacity-50" />
                  <p>{marketQuery.trim() ? t('marketplace.noResults') : t('marketplace.marketEmpty')}</p>
                </div>
              )}
              {!marketLoading &&
                marketResults.length > 0 &&
                marketTotal > marketResults.length && (
                  <div className="flex justify-center pt-6">
                    <Button
                      type="button"
                      variant="outline"
                      onClick={() => void searchMarketSkills(marketQuery.trim(), true)}
                      disabled={!isGatewayRunning}
                    >
                      {t('loadMoreMarket', { shown: marketResults.length, total: marketTotal })}
                    </Button>
                  </div>
                )}
            </div>
