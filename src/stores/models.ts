...
async function fetchModelsFromOneApi(tokenKey: string): Promise<OneApiModel[]> {
  ...
  if (!response.ok) {
    throw new Error(`OneAPI 请求失败: ${response.status}`);
  }
...
export const useModelsStore = create<ModelState>((set, get) => ({
  ...
  fetchModels: async () => {
    ...
    const tokenKey = await getTokenKey();
    console.log('[models] tokenKey:', tokenKey ? 'exists' : 'null');
    ...
    try {
      ...
      console.log('[models] fetched models:', models.length);
      ...
      set({
        models,
        loading: false,
        currentModelId: currentModelId || defaultModelId,
        isLoggedIn: true,
        error: null,
      });
    } catch (err) {
  ...
  logout: async () => {
    try {
      console.log('[models] logout called');
      const result = await invokeIpc<{ success: boolean; error?: string }>('box-im:logout');
      console.log('[models] logout result:', result);
  ...
  fetchDigitalEmployees: async () => {
    try {
      ...
      const employees = (data.bots || []) as DigitalEmployee[];
      console.log('[models] fetched digital employees:', employees.length);
      set({ digitalEmployees: employees });
    } catch (err) {
      console.warn('[models] Failed to fetch digital employees:', err);
      set({ digitalEmployees: [] });
    }
  },

  getAgentDefaultModel: (agentId: string): string | null => {
    const { digitalEmployees } = get();
    const employee = digitalEmployees.find(e => e.openclawAgentId === agentId);
    return employee?.model || null;
  },

  getSessionModel: (sessionKey: string): string | null => {
    const { sessionModels } = get();
    return sessionModels[sessionKey] || null;
  },
